...
class PayNowController extends Controller
{
    ...
    public function orderSave(Request $request){
       ...
       $cartqty=Cart::instance('shopping')->content()->count();
        ...
        // shipping calculte
          ...
          return redirect('customer/order/'.$order->id.'/payment');
    ...
    public function payment_pay(Request $request){
        ...
        return view('frontEnd.layouts.pages.payment_pay', compact('order_id','extrashipping'));
    }
    ...
    public function payment_pay_stripe(Request $request){
        ...
        $payment->paymentType    = $request->paymentType;
        $payment->senderId       = $request->holder_name;
        $payment->transectionId  = $data->balance_transaction;
        $payment->transFee       = $transFee;
        $payment->status         = 'Unpaid';
        $payment->save();

        Toastr::success('Thanks, Your order Save successfully', 'Success!');
        if ($request->system == 'api') {
